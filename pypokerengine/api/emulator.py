...
class Emulator(object):
    ...
    def apply_action(self, game_state, action, bet_amount=0):
        ...
        updated_state, messages = RoundManager.apply_action(
            game_state, action, bet_amount
        )
    ...
    def start_new_round(self, game_state, msg_summarizer=None, pocket_cards_map=None):
        ...
        ante, sb_amount = update_blind_level(
        ...
        is_game_finished = (
            len([1 for p in deepcopy_table.seats.players if p.is_active()]) == 1
        )
        if is_game_finished:
            return deepcopy, self._generate_game_result_event(deepcopy)

        new_state, messages = RoundManager.start_new_round(
            round_count, sb_amount, ante, deepcopy_table, pocket_cards_map
        )
        if msg_summarizer:
            msg_summarizer.summarize_messages(messages)
        events = [self.create_event(message[1]["message"]) for message in messages]
        events = [e for e in events if e]
    ...
    def _is_last_round(self, game_state, game_rule):
        ...
        is_winner_decided = (
            len([1 for p in game_state["table"].seats.players if p.stack != 0]) == 1
        )
    ...
    def _generate_game_result_event(self, game_state):
        ...
        message = MessageBuilder.build_game_result_message(
            dummy_config, game_state["table"].seats
        )["message"]
...
def _steal_money_from_poor_player(table, ante, sb_amount):
    ...
    bb_player = _find_first_elligible_player(
...
class Event:
    ...
    def create_game_finish_event(self, message):
        ...
        return {
            ...
            "players": [
                player_info(info) for info in message["game_information"]["seats"]
            ],
        }
