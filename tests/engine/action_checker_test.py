...
class ActionCheckerTest(BaseUnitTest):
  ...
  def test_check(self):
    ...
    self.false(self.checker._ActionChecker__is_illegal(players, 0, 'call', 0))
    self.eq(0, self.checker.need_amount_for_action(players[0], 0))
    self.eq(0, self.checker.need_amount_for_action(players[1], 0))

  def test_call(self):
    players = self.__setup_clean_players()
    self.true(self.checker._ActionChecker__is_illegal(players, 0, 'call', 10))

  def test_too_small_raise(self):
    players = self.__setup_clean_players()
    self.true(self.checker._ActionChecker__is_illegal(players, 0, 'raise', 4))

  def test_legal_raise(self):
    players = self.__setup_clean_players()
    self.false(self.checker._ActionChecker__is_illegal(players, 0, 'raise', 5))

  """ the case when agree amount = $10, minimum bet = $15"""
  def test__fold(self):
    players = self.__setup_blind_players()
    self.false(self.checker._ActionChecker__is_illegal(players, 0, 'fold'))

  def test__call(self):
    players = self.__setup_blind_players()
    self.true(self.checker._ActionChecker__is_illegal(players, 0, 'call', 9))
    self.false(self.checker._ActionChecker__is_illegal(players, 0, 'call', 10))
    self.true(self.checker._ActionChecker__is_illegal(players, 0, 'call', 11))
  ...
  def test__raise(self):
    players = self.__setup_blind_players()
    self.true(self.checker._ActionChecker__is_illegal(players, 0, 'raise', 14))
    self.false(self.checker._ActionChecker__is_illegal(players, 0, 'raise', 15))
    self.false(self.checker._ActionChecker__is_illegal(players, 0, 'raise', 16))
  ...
  def test__short_of_money(self):
    ...
    self.false(self.checker._ActionChecker__is_illegal(players, 0, "call", 10))
    self.true(self.checker._ActionChecker__is_illegal(players, 0, "call", 15))

  def test_small_blind_allin_raise(self):
    players = self.__setup_blind_players()
    self.false(self.checker._ActionChecker__is_illegal(players, 0, "raise", 100))

  def test_big_blind_allin_call(self):
    players = self.__setup_blind_players()
    players[0].add_action_history(Action.RAISE, 100, 95)
    self.false(self.checker._ActionChecker__is_illegal(players, 1, "call", 100))
    players[1].collect_bet(1)
    self.true(self.checker._ActionChecker__is_illegal(players, 1, "call", 100))

  def test_allin_check_on_call(self):
    player = self.__setup_clean_players()[0]
    self.false(self.checker.is_allin(player, "call", 99))
    self.true(self.checker.is_allin(player, "call", 100))
    self.true(self.checker.is_allin(player, "call", 101))

  def test_allin_check_on_raise(self):
    player = self.__setup_clean_players()[0]
    self.false(self.checker.is_allin(player, "raise", 99))
    self.true(self.checker.is_allin(player, "raise", 100))
    self.false(self.checker.is_allin(player, "raise", 101))

  def test_allin_check_on_fold(self):
    player = self.__setup_clean_players()[0]
    self.false(self.checker.is_allin(player, "fold", 0))

  def test_correct_action_on_allin_call(self):
    players = self.__setup_clean_players()
    players[0].add_action_history(Action.RAISE, 50, 50)
    players[1].stack = 30
    action, bet_amount = self.checker.correct_action(players, 1, 'call', 50)
    self.eq('call', action)
    self.eq(30, bet_amount)

  def test_correct_action_on_allin_raise(self):
    players = self.__setup_clean_players()
    action, bet_amount = self.checker.correct_action(players, 0, 'raise', 100)
    self.eq('raise', action)
    self.eq(100, bet_amount)

  def test_correct_illegal_call(self):
    players = self.__setup_clean_players()
    action, bet_amount = self.checker.correct_action(players, 0, 'call', 10)
    self.eq('fold', action)
    self.eq(0, bet_amount)

  def test_correct_illegal_raise(self):
    players = self.__setup_clean_players()
    action, bet_amount = self.checker.correct_action(players, 0, 'raise', 101)
    self.eq('fold', action)
    self.eq(0, bet_amount)

  def test_correct_action_when_legal(self):
    players = self.__setup_clean_players()
    action, bet_amount = self.checker.correct_action(players, 0, 'call', 0)
    self.eq('call', action)
    self.eq(0, bet_amount)
  ...
  def test_legal_actions(self):
    players = self.__setup_blind_players()
    legal_actions = self.checker.legal_actions(players, 0)
